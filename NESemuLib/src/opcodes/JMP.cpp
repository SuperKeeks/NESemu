...
int CPU::JMP(AddressingMode mode)
{
    switch (mode)
    {
        ...
        {
            const uint8_t addressLo = _memoryMapper->ReadMem(++_state._programCounter);
            const uint8_t addressHi = _memoryMapper->ReadMem(++_state._programCounter);
            ...
            // NOTE: Subtract 1 so next time PC is incremented it fetches the right opcode
            _state._programCounter = _memoryMapper->ReadMem(finalAddress1) + (_memoryMapper->ReadMem(finalAddress2) << 8) - 1;
